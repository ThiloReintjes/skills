...
5. Establish the collection contract from existing consent controls, privacy disclosures, and user requirements: allowed events, identity/person properties, persistence, IP/GeoIP, replay content, and AI content. Preserve existing restrictions. If the policy is unresolved, ask before enabling additional collection; continue work that does not depend on that decision.

Finish when each in-scope surface has an implementation location and collection policy, or a recorded reason it does not apply. Dashboard mutations, alert subscriptions, and production probes require appropriate access and authorization; missing access becomes a specific handoff, not permission to expand scope.
...
Use the trusted internal auth ID consistently wherever identification is permitted; mutable email is a person property, not a join key. Define anonymous and restricted operational identities explicitly instead of forcing all events into identified profiles. Keep consented or sampled funnels distinct from complete operational counts.

Route feature code through a typed analytics boundary. Make capture and flush failures non-fatal, bound delivery waits to the product's latency budget, and surface failures through sanitized, bounded logs. Exclude credentials, authorization headers, sensitive payment fields, and prohibited customer data. Apply the collection contract to serialized payloads, including URLs, exception messages, stack frames, and local variables—not just custom event properties. If free-form errors can contain customer content, retain safe type/phase/stack metadata instead of raw text.

Finish with event producers, correlation keys, permitted properties, and populations defined for each measured journey.

## 3. Implement browser coverage

When a browser is in scope, read [Browser lifecycle](references/browser-lifecycle.md) before implementing consent, identity, redirect delivery, and exception ownership.

Initialize at the earliest framework-supported root permitted by the collection contract. Configure autocapture, route-aware pageviews/page leaves, replay, heatmaps, dead clicks, and Web Vitals for the requested coverage. Assign one owner per automatic event to avoid duplicates. Mask sensitive rendered content and scrub sensitive URLs/network data before enabling replay; input masking alone does not protect ordinary page text.

Finish when the applicable lifecycle checks in the reference pass and enabled browser products obey the collection contract.
...
- attach the permitted identity/correlation keys plus consistent `service`, `runtime`, and `environment` properties;
...
Attribute location to the client only when the collection contract permits it and a trusted hosting boundary supplies the IP. Otherwise disable GeoIP so the datacenter is not mistaken for the user. Webhook/provider IPs are not user IPs.
...
When the product calls LLM or embedding APIs, use PostHog's official provider wrapper where available. Trace generations and embeddings, with spans for material stages such as retrieval, tool calls, and retries. Carry an operation trace ID through concurrent/background stages and a session ID through conversations; link permitted user/replay identities where supported.

Capture model, usage, cost, latency, time to first token where available, and sanitized error metadata. For sensitive inputs such as user documents, use [privacy mode](https://posthog.com/docs/ai-observability/privacy-mode) or equivalent metadata-only capture. Enable prompt/output content only when explicitly permitted by the collection contract. Check tool payloads, exception text, and captured locals separately; input/output suppression does not establish that the rest of an event is safe.

Verify emitted payloads with synthetic sensitive inputs and prove the provider call still works when telemetry is disabled or fails.
...
- Define each metric's population, denominator, environment, time window, and expected delay. Account for consent, sampling, retries, ingestion lag, and normal abandonment before labeling missing events as failures.
...
2. Add focused tests for the applicable checks above: serialized payloads/redaction, proxy routing, failure isolation and time bounds, exception ownership, terminal-event correlation, and runtime delivery. Exercise the browser lifecycle reference and AI trace propagation when applicable.
...
4. Perform authorized, synthetic end-to-end probes when configuration permits. Inspect received events, replays, exceptions, and AI traces for correct region, permitted identity/GeoIP, correlation, and redaction—not merely event presence. Include a failing path and a user-visible successful outcome.
5. Report changed files, configuration names, event/collection contracts, proxy route, monitor links or definitions, verification evidence, and specific external handoffs. Distinguish code complete from live delivery and notification verification.
